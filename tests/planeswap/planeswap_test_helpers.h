...
inline std::string yuy2_case_name(
    std::size_t width_bytes, std::size_t height, std::size_t source_pitch,
    std::size_t destination_pitch, const Variant<PlaneSwapFuncPtr>& variant) {
  std::ostringstream stream;
  stream << "Yuy2Swap_Width" << width_bytes
         << "_Height" << height
         << "_SrcPitch" << source_pitch
         << "_DstPitch" << destination_pitch
         << "_PatternChannelRamp_" << planeswap_variant_name(variant);
  return stream.str();
}

inline std::string rgb_case_name(const RgbExtractCase& test_case) {
  std::ostringstream stream;
  stream << test_case.format
         << "Channel" << planeswap_channel_name(test_case.channel_index)
...
inline Yuy2SwapCase make_yuy2_case(
    std::size_t width_bytes, std::size_t height, std::size_t source_pitch,
    std::size_t destination_pitch, Variant<PlaneSwapFuncPtr> variant,
    std::string expected_hash) {
  Yuy2SwapCase result{width_bytes, height, source_pitch, destination_pitch,
                      std::move(variant), std::move(expected_hash), {}};
  result.name = yuy2_case_name(result.width_bytes, result.height,
                               result.source_pitch, result.destination_pitch,
                               result.variant);
  return result;
}

inline RgbExtractCase make_rgb_case(
    std::string format, int channel_index, std::size_t width_pixels,
    std::size_t height, std::size_t source_pitch, std::size_t destination_pitch,
    std::size_t bytes_per_channel, PlaneSwapFuncPtr function,
    Variant<PlaneSwapFuncPtr> variant, std::string expected_hash) {
  RgbExtractCase result{std::move(format), channel_index, width_pixels, height,
                        source_pitch, destination_pitch, bytes_per_channel,
                        function, std::move(variant), std::move(expected_hash), {}};
...
inline void run_yuy2_case(const Yuy2SwapCase& test_case) {
  GuardedVideoBuffer<std::uint8_t> source(
      test_case.width_bytes, test_case.height, test_case.source_pitch, 32);
  GuardedVideoBuffer<std::uint8_t> expected(
      test_case.width_bytes, test_case.height, test_case.destination_pitch, 32);
  ...
  for (std::size_t y = 0; y < view.height(); ++y) {
    for (std::size_t x = 0; x < width_pixels; ++x) {
      for (std::size_t channel = 0; channel < kComponents; ++channel) {
        ...
        view.row(y)[x * kComponents + channel] =
            static_cast<T>(value & max_value);
      }
    }
  }
}
...
template <typename T>
void apply_rgb_reference(const RgbExtractCase& test_case,
  ...
  for (std::size_t output_y = 0; output_y < test_case.height; ++output_y) {
    ...
    for (std::size_t x = 0; x < test_case.width_pixels; ++x) {
      destination.row(output_y)[x] =
          source.row(source_y)[x * 4 + test_case.channel_index];
    }
  }
}
...
void run_rgb_case_typed(const RgbExtractCase& test_case) {
  ...
  GuardedVideoBuffer<T> expected(
      test_case.width_pixels, test_case.height, test_case.destination_pitch, 64);
