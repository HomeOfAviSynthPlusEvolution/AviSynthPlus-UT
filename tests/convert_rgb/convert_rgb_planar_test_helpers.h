...
using PackedToPlanarRgbFunction =
    void (*)(const BYTE*, BYTE* (&)[4], int, int (&)[4], int, int);
...
inline std::string packed_to_planar_rgb_case_name(
    const PackedToPlanarRgbCase& test_case) {
  std::ostringstream stream;
  stream << test_case.operation << "_Width" << test_case.width_pixels
         << "_Height" << test_case.height << "_SrcPitch"
         << test_case.source_pitch << "_DstPitches"
         << test_case.destination_pitches[0] << '_'
         << test_case.destination_pitches[1] << '_'
         << test_case.destination_pitches[2] << '_'
         << test_case.destination_pitches[3]
...
inline PackedToPlanarRgbCase make_packed_to_planar_rgb_case(
    ...
    std::size_t source_components, bool target_has_alpha,
    std::size_t width_pixels, std::size_t height,
    std::size_t source_pitch,
    std::array<std::size_t, 4> destination_pitches,
    Variant<PackedToPlanarRgbFunction> variant,
    std::array<std::string, 4> expected_hashes) {
  PackedToPlanarRgbCase result{
      std::move(operation), bytes_per_component, source_components,
      target_has_alpha, width_pixels, height, source_pitch,
      std::move(destination_pitches), std::move(variant),
      std::move(expected_hashes), {}};
  result.name = packed_to_planar_rgb_case_name(result);
  return result;
}

inline void PrintTo(const PackedToPlanarRgbCase& test_case,
                    std::ostream* stream) {
  *stream << test_case.name;
}

template <typename T>
void make_packed_to_planar_rgb_reference(
    PlaneView<const T> source, std::array<PlaneView<T>, 4> destination,
    const PackedToPlanarRgbCase& test_case) {
  ...
  for (std::size_t y = 0; y < test_case.height; ++y) {
    ...
    for (std::size_t x = 0; x < test_case.width_pixels; ++x) {
      const auto* source_pixel =
          source_row + x * test_case.source_components;
      ...
      if (test_case.target_has_alpha) {
        destination[3].row(y)[x] = test_case.source_components == 4
                                       ? source_pixel[3]
                                       : opaque_alpha;
      }
    }
...
void run_packed_to_planar_rgb_case(const PackedToPlanarRgbCase& test_case) {
  ...
  make_packed_to_planar_rgb_reference(
      ...
      {expected_g.view(), expected_b.view(), expected_r.view(),
       expected_a.view()},
      test_case);
  ...
  int destination_pitches[4]{
      static_cast<int>(actual_g.view().pitch_bytes()),
  ...
  const std::array<PlaneView<const T>, 4> expected{
      expected_g.view().as_const(), expected_b.view().as_const(),
      expected_r.view().as_const(), expected_a.view().as_const()};
  const std::array<PlaneView<const T>, 4> actual{
      actual_g.view().as_const(), actual_b.view().as_const(),
      actual_r.view().as_const(), actual_a.view().as_const()};
  ...
  for (std::size_t plane = 0; plane < plane_count; ++plane) {
    EXPECT_TRUE(compare_exact(expected[plane], actual[plane]))
        ...
        << test_case.variant.name << " plane " << kPlaneNames[plane];
    EXPECT_EQ(format_hash(hash_active(expected[plane])),
              test_case.expected_hashes[plane])
        << test_case.name << " stable output hash mismatch for plane "
        << kPlaneNames[plane];
  }
  if (!test_case.target_has_alpha) {
