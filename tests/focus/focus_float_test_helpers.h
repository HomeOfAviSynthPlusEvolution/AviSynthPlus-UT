...
using FocusHorizontalFloatFuncPtr = void (*)(BYTE*, std::size_t, std::size_t,
                                             std::size_t, float);
...
inline std::string focus_float_case_name(
    const char* format, std::size_t width, std::size_t height,
  ...
  stream << format << "_Width" << width
         ...
         << "_Pitch" << pitch
         << "_Amount" << amount_name
         << "_PatternFiniteAnchors_" << focus_variant_name(variant);
  return stream.str();
}

inline FocusHorizontalFloatCase make_focus_horizontal_float_case(
    std::size_t width, std::size_t height, std::size_t pitch, float amount,
    std::string amount_name, Variant<FocusHorizontalFloatFuncPtr> variant) {
  FocusHorizontalFloatCase result{width, height, pitch, amount,
                                  std::move(amount_name), std::move(variant), {}};
  result.name = focus_float_case_name(
      "PlaneFloatHorizontal", result.width, result.height, result.pitch,
...
inline FocusVerticalFloatCase make_focus_vertical_float_case(
    std::size_t width, std::size_t height, std::size_t pitch, float amount,
    std::string amount_name, Variant<FocusVerticalFloatFuncPtr> variant) {
  FocusVerticalFloatCase result{width, height, pitch, amount,
                                std::move(amount_name), std::move(variant), {}};
  result.name = focus_float_case_name(
      "PlaneFloatVertical", result.width, result.height, result.pitch,
      result.amount_name, result.variant);
  return result;
}

inline void PrintTo(const FocusHorizontalFloatCase& test_case,
                    std::ostream* stream) {
  *stream << test_case.name;
}

inline void PrintTo(const FocusVerticalFloatCase& test_case,
                    std::ostream* stream) {
  *stream << test_case.name;
}

inline void fill_focus_float_input(PlaneView<float> view) {
  constexpr std::array<float, 9> anchors{
      -1000.0F, -64.0F, -1.0F, -0.5F, 0.0F,
...
inline float focus_float_reference_pixel(float left, float center, float right,
                                         float amount) {
...
inline ::testing::AssertionResult compare_focus_float(
    ...
    for (std::size_t x = 0; x < expected.width(); ++x) {
      ...
      const auto ulps = (lhs_bits & sign_bit) != (rhs_bits & sign_bit)
                            ...
                            : static_cast<std::uint64_t>(
                                  lhs_bits >= rhs_bits ? lhs_bits - rhs_bits
                                                       : rhs_bits - lhs_bits);
...
inline void apply_focus_horizontal_float_reference(
    PlaneView<const float> source, PlaneView<float> destination, float amount) {
  ...
  for (std::size_t y = 0; y < source.height(); ++y) {
    for (std::size_t x = 0; x < source.width(); ++x) {
      ...
      destination.row(y)[x] = focus_float_reference_pixel(
          left, source.row(y)[x], right, amount);
    }
  }
}

inline void apply_focus_vertical_float_reference(
    PlaneView<const float> source, PlaneView<float> destination, float amount) {
  ASSERT_EQ(source.width(), destination.width());
  ASSERT_EQ(source.height(), destination.height());
  for (std::size_t y = 0; y < source.height(); ++y) {
    ...
    for (std::size_t x = 0; x < source.width(); ++x) {
      destination.row(y)[x] = focus_float_reference_pixel(
          upper[x], center[x], lower[x], amount);
    }
  }
}

inline void run_focus_horizontal_float_case(
    const FocusHorizontalFloatCase& test_case) {
  GuardedVideoBuffer<float> input(
  ...
  EXPECT_TRUE(compare_focus_float(expected.view().as_const(),
...
inline void run_focus_vertical_float_case(
    const FocusVerticalFloatCase& test_case) {
  ...
  apply_focus_vertical_float_reference(actual.view().as_const(), expected.view(),
                                       test_case.amount);
  std::copy_n(reinterpret_cast<const std::uint8_t*>(actual.view().data()),
              active_row_bytes, line_buffer.view().data());

  test_case.variant.function(
      line_buffer.view().data(), reinterpret_cast<BYTE*>(actual.view().data()),
