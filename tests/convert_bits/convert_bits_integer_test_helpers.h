...
using ConvertBitsIntegerFunc = void (*)(const BYTE*, BYTE*, int, int, int,
                                        int, int, int, int);

enum class IntegerStorage { UInt8ToUInt8, UInt8ToUInt16, UInt16ToUInt8,
                            UInt16ToUInt16 };
...
inline void PrintTo(const ConvertBitsIntegerCase& test_case,
                    std::ostream* stream) {
...
inline const char* integer_storage_name(IntegerStorage storage) {
  switch (storage) {
    case IntegerStorage::UInt8ToUInt8: return "UInt8ToUInt8";
    case IntegerStorage::UInt8ToUInt16: return "UInt8ToUInt16";
    case IntegerStorage::UInt16ToUInt8: return "UInt16ToUInt8";
    case IntegerStorage::UInt16ToUInt16: return "UInt16ToUInt16";
  }
  return "Unknown";
}
...
inline ConvertBitsIntegerCase make_convert_bits_integer_case(
    ...
    int source_bit_depth, int target_bit_depth, std::size_t width,
    std::size_t height, std::size_t source_pitch, std::size_t target_pitch,
    Variant<ConvertBitsIntegerFunc> variant, std::string expected_hash) {
  ...
  stream << (chroma ? "Chroma" : "Luma") << "_Src" << source_bit_depth
         ...
         << "_Storage" << integer_storage_name(storage) << "_Width" << width
         << "_Height" << height << "_SrcPitch" << source_pitch
         << "_DstPitch" << target_pitch
         << "_DitherNone_PatternBoundaryValues_"
...
inline IntegerRange integer_range(bool chroma, bool full, int bit_depth) {
  ...
  if (chroma) {
    return {static_cast<double>(std::uint32_t{1} << (bit_depth - 1)),
            full ? maximum / 2.0
                 : static_cast<double>(112U << (bit_depth - 8))};
  }
  return {full ? 0.0
               : static_cast<double>(16U << (bit_depth - 8)),
          full ? maximum
               : static_cast<double>(219U << (bit_depth - 8))};
}
...
void fill_convert_bits_integer_source(PlaneView<T> source,
  ...
  const auto range = integer_range(test_case.chroma, test_case.source_full,
                                   test_case.source_bit_depth);
  const auto lower = static_cast<std::uint32_t>(
      std::max(0.0, std::ceil(range.offset - range.span)));
  const auto upper = static_cast<std::uint32_t>(
      std::min(static_cast<double>(maximum),
               std::floor(range.offset + range.span)));
  ...
  for (std::size_t y = 0; y < source.height(); ++y) {
    for (std::size_t x = 0; x < source.width(); ++x) {
      source.row(y)[x] =
          static_cast<T>(anchors[(x + 5 * y) % anchors.size()]);
    }
  }
}

inline std::uint32_t convert_bits_integer_reference(
    std::uint32_t source, const ConvertBitsIntegerCase& test_case) {
  ...
  if (!test_case.source_full && !test_case.target_full) {
    if (test_case.target_bit_depth > test_case.source_bit_depth) {
      return source <<
             (test_case.target_bit_depth - test_case.source_bit_depth);
    }
  ...
  const double mapped =
      (static_cast<double>(source) - source_range.offset) *
  ...
  return static_cast<std::uint32_t>(std::clamp<std::int64_t>(
      rounded, 0, static_cast<std::int64_t>(target_maximum)));
}

template <typename Source, typename Target>
void run_convert_bits_integer_case_typed(
    const ConvertBitsIntegerCase& test_case) {
  ...
  for (std::size_t y = 0; y < test_case.height; ++y) {
    for (std::size_t x = 0; x < test_case.width; ++x) {
      expected.view().row(y)[x] = static_cast<Target>(
          convert_bits_integer_reference(source.view().row(y)[x], test_case));
    }
  }
  ...
  EXPECT_TRUE(source.active_matches(source_snapshot))
      << test_case.name << " modified the source";
...
inline void run_convert_bits_integer_case(
    const ConvertBitsIntegerCase& test_case) {
  switch (test_case.storage) {
    ...
    case IntegerStorage::UInt16ToUInt16:
      run_convert_bits_integer_case_typed<std::uint16_t, std::uint16_t>(
          test_case);
      break;
  }
}
...
template <typename Source, typename Target, bool Chroma, bool SourceFull,
          bool TargetFull>
void add_convert_bits_integer_variants(
    std::vector<ConvertBitsIntegerCase>& cases, IntegerStorage storage,
    int source_bit_depth, int target_bit_depth, std::size_t source_pitch,
  ...
  cases.push_back(make_convert_bits_integer_case(
      ...
      target_bit_depth, width, height, source_pitch, target_pitch,
      Variant<ConvertBitsIntegerFunc>{
          "sse41",
          convert_uint_sse41<Source, Target, Chroma, SourceFull, TargetFull>,
  ...
  cases.push_back(make_convert_bits_integer_case(
      ...
      Variant<ConvertBitsIntegerFunc>{
          "avx2",
          convert_uint_avx2<Source, Target, Chroma, SourceFull, TargetFull>,
          IsaRequirement::Avx2},
      expected_hash));
}
