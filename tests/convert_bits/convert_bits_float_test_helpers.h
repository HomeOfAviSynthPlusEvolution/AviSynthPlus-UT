...
enum class FloatConversion { FloatToUInt8, FloatToUInt16, UInt8ToFloat,
                             UInt16ToFloat };
...
inline void PrintTo(const ConvertBitsFloatCase& test_case,
                    std::ostream* stream) {
...
inline const char* float_conversion_name(FloatConversion conversion) {
  switch (conversion) {
    case FloatConversion::FloatToUInt8: return "FloatToUInt8";
    case FloatConversion::FloatToUInt16: return "FloatToUInt16";
    case FloatConversion::UInt8ToFloat: return "UInt8ToFloat";
    case FloatConversion::UInt16ToFloat: return "UInt16ToFloat";
  }
  return "Unknown";
}

inline ConvertBitsFloatCase make_convert_bits_float_case(
    FloatConversion conversion, bool chroma, bool source_full,
    bool target_full, int source_bit_depth, int target_bit_depth,
  ...
  ConvertBitsFloatCase result{
      conversion,       chroma,          source_full, target_full,
      source_bit_depth, target_bit_depth, width,       height,
      source_pitch,     target_pitch,    std::move(variant),
      std::move(expected_hash), {}};
  std::ostringstream stream;
  stream << (chroma ? "Chroma" : "Luma") << "_"
         << float_conversion_name(conversion) << "_Src" << source_bit_depth
         << (source_full ? "Full" : "Limited") << "_Dst"
         << target_bit_depth << (target_full ? "Full" : "Limited")
         << "_Width" << width << "_Height" << height << "_SrcPitch"
         << source_pitch << "_DstPitch" << target_pitch
         << "_PatternFiniteAnchors_"
...
inline void fill_float_source(PlaneView<float> source, bool chroma) {
  constexpr std::array<float, 16> luma_anchors{
      -0.25F, 0.0F, 1.0F / 255.0F, 15.0F / 255.0F,
      16.0F / 255.0F, 17.0F / 255.0F, 0.25F, 0.5F,
      0.75F, 234.0F / 255.0F, 235.0F / 255.0F,
      236.0F / 255.0F, 254.0F / 255.0F, 1.0F, 1.25F, 0.125F};
  constexpr std::array<float, 16> chroma_anchors{
      -0.75F, -0.5F, -112.0F / 255.0F, -0.25F,
      -1.0F / 255.0F, 0.0F, 1.0F / 255.0F, 0.25F,
      112.0F / 255.0F, 0.5F, 0.75F, -0.125F,
...
inline std::uint32_t float_to_integer_reference(
    float source, const ConvertBitsFloatCase& test_case) {
  const auto source_range = numeric_range(
      test_case.chroma, test_case.source_full, test_case.source_bit_depth);
  const auto target_range = numeric_range(
      test_case.chroma, test_case.target_full, test_case.target_bit_depth);
  const float mapped =
      (source - source_range.offset) *
          (target_range.span / source_range.span) +
      target_range.offset + 0.5F;
  const float target_maximum =
      static_cast<float>(bit_depth_max(test_case.target_bit_depth));
  return static_cast<std::uint32_t>(
      std::clamp(mapped, 0.0F, target_maximum));
}

inline float integer_to_float_reference(
    std::uint32_t source, const ConvertBitsFloatCase& test_case) {
  const auto source_range = numeric_range(
      test_case.chroma, test_case.source_full, test_case.source_bit_depth);
  const auto target_range = numeric_range(
      test_case.chroma, test_case.target_full, test_case.target_bit_depth);
  return static_cast<float>(
      (static_cast<double>(source) - static_cast<double>(source_range.offset)) *
          (static_cast<double>(target_range.span) /
           static_cast<double>(source_range.span)) +
      static_cast<double>(target_range.offset));
}

inline std::uint64_t convert_bits_ulp_distance(float lhs,
                                               float rhs) noexcept {
  if (lhs == rhs) {
    return 0;
  }
...
inline ::testing::AssertionResult compare_convert_bits_float(
...
void run_float_to_integer_case(const ConvertBitsFloatCase& test_case) {
  GuardedVideoBuffer<float> source(test_case.width, test_case.height,
  ...
  test_case.variant.function(
      ...
      test_case.target_bit_depth, test_case.target_bit_depth);
...
void run_integer_to_float_case(const ConvertBitsFloatCase& test_case) {
  GuardedVideoBuffer<Source> source(test_case.width, test_case.height,
                                    test_case.source_pitch, 64);
  GuardedVideoBuffer<float> expected(test_case.width, test_case.height,
                                     test_case.target_pitch, 64);
  GuardedVideoBuffer<float> actual(test_case.width, test_case.height,
                                   test_case.target_pitch, 64);
  ConvertBitsIntegerCase input_case{
      std::is_same_v<Source, std::uint8_t> ? IntegerStorage::UInt8ToUInt8
  ...
  for (std::size_t y = 0; y < test_case.height; ++y) {
    for (std::size_t x = 0; x < test_case.width; ++x) {
      expected.view().row(y)[x] = integer_to_float_reference(
          source.view().row(y)[x], test_case);
    }
  }
  test_case.variant.function(
      reinterpret_cast<const BYTE*>(source.view().data()),
      ...
      static_cast<int>(test_case.target_pitch), test_case.source_bit_depth,
      test_case.target_bit_depth, test_case.target_bit_depth);
  EXPECT_TRUE(compare_convert_bits_float(expected.view().as_const(),
...
inline void run_convert_bits_float_case(
...
void add_float_to_integer_variants(
    std::vector<ConvertBitsFloatCase>& cases, FloatConversion conversion,
    int target_bit_depth, const char* expected_hash) {
  ...
  cases.push_back(make_convert_bits_float_case(
      conversion, Chroma, SourceFull, TargetFull, 32, target_bit_depth, width,
      height, source_pitch, target_pitch,
      Variant<ConvertBitsIntegerFunc>{
          "sse41",
          convert_32_to_uintN_sse41<Target, Chroma, SourceFull, TargetFull>,
  ...
  cases.push_back(make_convert_bits_float_case(
      ...
      Variant<ConvertBitsIntegerFunc>{
          "avx2-fma",
          convert_32_to_uintN_avx2<Target, Chroma, SourceFull, TargetFull>,
          IsaRequirement::Avx2Fma},
      expected_hash));
}

template <typename Source, bool Chroma, bool SourceFull, bool TargetFull>
void add_integer_to_float_case(std::vector<ConvertBitsFloatCase>& cases,
                               FloatConversion conversion,
                               int source_bit_depth,
                               std::size_t source_pitch) {
  cases.push_back(make_convert_bits_float_case(
      conversion, Chroma, SourceFull, TargetFull, source_bit_depth, 32, 32, 5,
      source_pitch, 160,
      Variant<ConvertBitsIntegerFunc>{
          "avx2-fma",
          convert_uintN_to_float_avx2<Source, Chroma, SourceFull, TargetFull>,
          IsaRequirement::Avx2Fma}));
}
