...
std::vector<GreyscaleYuy2Case> greyscale_yuy2_cases() {
  return {make_greyscale_yuy2_case(
      64, 5, 64,
      Variant<GreyscaleYuy2FuncPtr>{"sse2", greyscale_yuy2_sse2,
                                    IsaRequirement::Sse2},
      "456f296978afdfbb")};
}
...
std::vector<GreyscaleRgb32Case> greyscale_rgb32_cases() {
  ...
  for (const auto& item : {std::tuple{"Rec601", "Limited", rec601, limited},
    ...
    cases.push_back(make_greyscale_rgb32_case(
        ...
        std::get<3>(item), 12, 5, 48,
        Variant<GreyscaleRgb32FuncPtr>{"sse2", greyscale_rgb32_sse2,
                                       IsaRequirement::Sse2},
        std::get<2>(item) == rec601 ? "599e57d273252700"
                                    : "5bb635aba3c3792a"));
  }
  return cases;
}
...
std::vector<GreyscaleRgb64Case> greyscale_rgb64_cases() {
  ...
  for (const auto& item : {std::tuple{"Rec601", "Limited", rec601, limited},
    ...
    cases.push_back(make_greyscale_rgb64_case(
        std::get<0>(item), std::get<1>(item), std::get<2>(item),
        std::get<3>(item), 8, 5, 64,
        Variant<GreyscaleRgb64FuncPtr>{"sse41", greyscale_rgb64_sse41,
                                       IsaRequirement::Sse41},
        std::get<2>(item) == rec601 ? "8e86848b4bcc9fb5"
                                    : "5a58fd5f9ff4b1ac"));
  }
  return cases;
}
...
INSTANTIATE_TEST_SUITE_P(
    Kernels,
    GreyscaleYuy2Kernels,
    ::testing::ValuesIn(greyscale_yuy2_cases()),
