...
using LayerPlanarRgbLightenDarkenFloatFunction = layer_planarrgb_lighten_darken_f_c_t*;

struct LayerPlanarRgbLightenDarkenFloatCase {
  bool lighten{};
  bool has_alpha{};
  bool blend_alpha{};
  ...
  Variant<LayerPlanarRgbLightenDarkenFloatFunction> variant;
  std::string name;
};

inline LayerPlanarRgbLightenDarkenFloatFunction
layer_planarrgb_lighten_darken_float_function(bool lighten, bool has_alpha, bool blend_alpha) {
  layer_planarrgb_lighten_darken_c_t* integer_function = nullptr;
  LayerPlanarRgbLightenDarkenFloatFunction function = nullptr;
  get_layer_planarrgb_lighten_darken_functions_avx2(
      lighten, has_alpha, blend_alpha, 32, &integer_function, &function);
  return function;
}

inline std::string layer_planarrgb_lighten_darken_float_variant_name(
    const Variant<LayerPlanarRgbLightenDarkenFloatFunction>& variant) {
...
inline LayerPlanarRgbLightenDarkenFloatCase make_layer_planarrgb_lighten_darken_float_case(
    bool lighten, bool has_alpha, bool blend_alpha, std::size_t width_samples,
    std::size_t height, std::size_t destination_pitch, std::size_t overlay_pitch,
    std::size_t mask_pitch, float opacity, std::string opacity_name, float threshold,
    std::string threshold_name) {
  if (width_samples == 0 || height == 0 ||
      ...
      threshold < 0.0F || threshold > 1.0F || (blend_alpha && !has_alpha) ||
      (has_alpha && mask_pitch < width_samples * sizeof(float))) {
    throw std::invalid_argument("invalid float Layer planar RGB lighten/darken dimensions");
  }
  LayerPlanarRgbLightenDarkenFloatCase result{
      lighten,
      has_alpha,
      blend_alpha,
      width_samples,
      ...
      Variant<LayerPlanarRgbLightenDarkenFloatFunction>{
          "avx2", layer_planarrgb_lighten_darken_float_function(lighten, has_alpha, blend_alpha),
          IsaRequirement::Avx2},
      {}};
  std::ostringstream stream;
  stream << "Base" << (blend_alpha ? "Rgba" : "Rgb") << "_Overlay"
         << (has_alpha ? "Rgba" : "Rgb") << (lighten ? "_Lighten" : "_Darken")
  ...
  stream << "_Opacity" << result.opacity_name << "_Threshold" << result.threshold_name
         << "_PatternFiniteAnchors_"
         << layer_planarrgb_lighten_darken_float_variant_name(result.variant);
  result.name = stream.str();
  return result;
}

inline void PrintTo(const LayerPlanarRgbLightenDarkenFloatCase& test_case,
                    std::ostream* stream) {
  *stream << test_case.name;
}

template <typename T>
void fill_layer_planarrgb_lighten_darken_float_inputs(
    const LayerPlanarRgbLightenDarkenFloatCase& test_case,
...
template <typename T>
void copy_layer_planarrgb_lighten_darken_float_planes(
    const LayerPlanarRgbLightenDarkenFloatCase& test_case,
...
void apply_layer_planarrgb_lighten_darken_float_reference(
    const LayerPlanarRgbLightenDarkenFloatCase& test_case,
  ...
  for (std::size_t y = 0; y < test_case.height; ++y) {
    for (std::size_t x = 0; x < test_case.width_samples; ++x) {
      ...
      const auto destination_luma =
          kCyb * destination[1]->view().row(y)[x] + kCyg * destination[0]->view().row(y)[x] +
          kCyr * destination[2]->view().row(y)[x];
      ...
      const bool selected = test_case.lighten
                                ...
                                : overlay_luma < destination_luma - test_case.threshold;
      const auto effective_alpha = selected ? alpha : 0.0;
      for (std::size_t plane = 0; plane < plane_count; ++plane) {
        const auto destination_value = static_cast<double>(destination[plane]->view().row(y)[x]);
        const auto target = static_cast<double>(overlay[plane]->view().row(y)[x]);
        destination[plane]->view().row(y)[x] =
            static_cast<float>(destination_value + (target - destination_value) * effective_alpha);
      }
    }
  }
}

void run_layer_planarrgb_lighten_darken_float_case(
    const LayerPlanarRgbLightenDarkenFloatCase& test_case) {
  const auto destination_planes = test_case.blend_alpha ? std::size_t{4} : std::size_t{3};
  const auto overlay_planes = test_case.has_alpha ? std::size_t{4} : std::size_t{3};
  std::array<std::unique_ptr<GuardedVideoBuffer<float>>, 4> destination;
  ...
  fill_layer_planarrgb_lighten_darken_float_inputs(test_case, destination, overlay, mask);
  copy_layer_planarrgb_lighten_darken_float_planes(test_case, destination, expected);
  copy_layer_planarrgb_lighten_darken_float_planes(test_case, destination, actual);
  apply_layer_planarrgb_lighten_darken_float_reference(test_case, overlay, mask, expected);
  ...
  ASSERT_NE(test_case.variant.function, nullptr)
      << test_case.name << " AVX2 getter returned no float Planar RGB lighten/darken function";
