...
using LayerRgb32MulFunction = void (*)(BYTE*, const BYTE*, int, int, int, int, int);

struct LayerRgb32MulCase {
  bool use_chroma{};
...
inline std::string layer_rgb32_mul_variant_name(const Variant<LayerRgb32MulFunction>& variant) {
...
inline std::string layer_rgb32_mul_case_name(const LayerRgb32MulCase& test_case) {
...
inline LayerRgb32MulCase make_layer_rgb32_mul_case(
  ...
  if (level < 0 || level > 257) {
    throw std::invalid_argument("Layer RGB32 multiply level must be in 0..257");
  }
  LayerRgb32MulCase result{use_chroma,
                           width_pixels,
                           height,
                           destination_pitch,
                           overlay_pitch,
...
inline void PrintTo(const LayerRgb32MulCase& test_case, std::ostream* stream) {
...
inline void fill_layer_rgb32_mul_inputs(PlaneView<std::uint8_t> destination,
  ...
  for (std::size_t y = 0; y < destination.height(); ++y) {
    for (std::size_t x = 0; x < destination.width() / 4; ++x) {
...
inline void apply_layer_rgb32_mul_reference(const LayerRgb32MulCase& test_case,
  ...
  for (std::size_t y = 0; y < test_case.height; ++y) {
    for (std::size_t x = 0; x < test_case.width_pixels; ++x) {
      ...
      for (std::size_t channel = 0; channel < 4; ++channel) {
        const auto multiplier = test_case.use_chroma ? static_cast<int>(ovr[channel]) : luma;
        const auto multiplied = (multiplier * dst[channel]) >> 8;
        dst[channel] =
            static_cast<std::uint8_t>(dst[channel] + (((multiplied - dst[channel]) * alpha) >> 8));
      }
    }
  }
}

inline void run_layer_rgb32_mul_case(const LayerRgb32MulCase& test_case) {
  ...
  apply_layer_rgb32_mul_reference(test_case, overlay.view().as_const(), expected.view());
