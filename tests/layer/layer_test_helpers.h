...
inline const char* layer_mask_mode_name(MaskMode mode) {
  switch (mode) {
    ...
    case MASK420_MPEG2: return "Mask420Mpeg2";
    case MASK420_TOPLEFT: return "Mask420TopLeft";
    case MASK422: return "Mask422";
    case MASK422_MPEG2: return "Mask422Mpeg2";
    case MASK422_TOPLEFT: return "Mask422TopLeft";
...
inline const char* layer_placement_name(int placement) {
  switch (placement) {
    case PLACEMENT_MPEG1: return "Mpeg1";
    case PLACEMENT_MPEG2: return "Mpeg2";
    case PLACEMENT_TOPLEFT: return "TopLeft";
...
inline std::string layer_yuv_add_case_name(const LayerYuvAddCase& test_case) {
  ...
  stream << layer_mask_mode_name(test_case.mask_mode)
         << "_" << layer_placement_name(test_case.placement)
...
inline MaskMode layer_mask_mode(bool is_chroma, int colorspace, int placement) {
  ...
  if (colorspace == 420) {
    return placement == PLACEMENT_MPEG1
               ? MASK420
...
inline std::size_t layer_mask_height(MaskMode mode, std::size_t height) {
  return mode == MASK420 || mode == MASK420_MPEG2 || mode == MASK420_TOPLEFT
             ? height * 2
             : height;
}
...
inline VideoInfo layer_video_info(int colorspace, int bits_per_pixel) {
  ...
  if (colorspace == 420) {
    vi.pixel_type = bits_per_pixel == 8 ? VideoInfo::CS_YV12
                                        : VideoInfo::CS_YUV420P16;
  } else {
    vi.pixel_type = bits_per_pixel == 8 ? VideoInfo::CS_YV16
                                        : VideoInfo::CS_YUV422P16;
  }
  return vi;
}

inline LayerYuvAddFuncPtr layer_sse41_function(
    bool is_chroma, int colorspace, int placement, int bits_per_pixel) {
  ...
  get_layer_yuv_masked_add_functions_sse41(
      is_chroma, placement, vi, bits_per_pixel, &function, &float_function);
  return function;
}

inline LayerYuvAddFuncPtr layer_avx2_function(
    bool is_chroma, int colorspace, int placement, int bits_per_pixel) {
  auto vi = layer_video_info(colorspace, bits_per_pixel);
  masked_merge_fn_t* function = nullptr;
  masked_merge_float_fn_t* float_function = nullptr;
  get_layer_yuv_masked_add_functions_avx2(
      is_chroma, placement, vi, bits_per_pixel, &function, &float_function);
  return function;
}

inline LayerYuvAddCase make_layer_yuv_add_case(
    bool is_chroma, int colorspace, int placement, int bits_per_pixel,
    std::size_t width_pixels, std::size_t height_pixels, int opacity,
    std::string opacity_name, Variant<LayerYuvAddFuncPtr> variant,
    std::string expected_hash) {
...
template <typename T>
void fill_layer_plane(PlaneView<T> view, std::uint32_t max_value,
                      std::size_t offset) {
  constexpr std::array<std::uint32_t, 10> anchors{
      0, 1, 64, 127, 128, 192, 254, 255, 511, 1023};
  for (std::size_t y = 0; y < view.height(); ++y) {
    for (std::size_t x = 0; x < view.width(); ++x) {
      ...
      view.row(y)[x] = static_cast<T>(
          max_value == 255 ? normalized : (normalized * max_value) / 255U);
    }
  }
}

template <typename T>
inline std::uint32_t layer_effective_mask(const LayerYuvAddCase& test_case,
  ...
  switch (test_case.mask_mode) {
    ...
    case MASK420: {
      ...
      raw = (first[x * 2] + first[x * 2 + 1] + second[x * 2] +
             second[x * 2 + 1] + 2U) >> 2;
      break;
    }
    case MASK420_MPEG2: {
  ...
  return (raw * static_cast<std::uint32_t>(test_case.opacity) + max_value / 2U) /
         max_value;
}

template <typename T>
void apply_layer_reference(const LayerYuvAddCase& test_case,
  ...
  for (std::size_t y = 0; y < test_case.height_pixels; ++y) {
    for (std::size_t x = 0; x < test_case.width_pixels; ++x) {
      ...
      destination.row(y)[x] = static_cast<T>(
          (first * (max_value - effective) + second * effective + max_value / 2U) /
          max_value);
    }
  }
}
...
void run_layer_yuv_add_case(const LayerYuvAddCase& test_case) {
  ...
  for (std::size_t y = 0; y < destination.view().height(); ++y) {
    std::copy_n(destination.view().row(y), destination.view().width(),
                expected.view().row(y));
    std::copy_n(destination.view().row(y), destination.view().width(),
                actual.view().row(y));
  }
