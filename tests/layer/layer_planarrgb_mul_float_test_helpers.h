...
using LayerPlanarRgbMulFloatFunction = layer_planarrgb_mul_f_c_t*;

struct LayerPlanarRgbMulFloatCase {
  bool chroma{};
  bool has_alpha{};
  bool blend_alpha{};
  ...
  Variant<LayerPlanarRgbMulFloatFunction> variant;
  std::string name;
};

inline LayerPlanarRgbMulFloatFunction layer_planarrgb_mul_float_function(bool chroma,
                                                                         bool has_alpha,
                                                                         bool blend_alpha) {
  layer_planarrgb_mul_c_t* integer_function = nullptr;
  LayerPlanarRgbMulFloatFunction function = nullptr;
  get_layer_planarrgb_mul_functions_avx2(chroma, has_alpha, blend_alpha, 32, &integer_function,
                                         &function);
  return function;
}

inline std::string layer_planarrgb_mul_float_variant_name(
    const Variant<LayerPlanarRgbMulFloatFunction>& variant) {
...
inline LayerPlanarRgbMulFloatCase make_layer_planarrgb_mul_float_case(
  ...
  if (width_samples == 0 || height == 0 ||
    ...
    throw std::invalid_argument("invalid float Layer planar RGB multiply dimensions");
  }
  LayerPlanarRgbMulFloatCase result{
      chroma,
      has_alpha,
      blend_alpha,
      width_samples,
      ...
      Variant<LayerPlanarRgbMulFloatFunction>{
          "avx2", layer_planarrgb_mul_float_function(chroma, has_alpha, blend_alpha),
          IsaRequirement::Avx2},
      {}};
  ...
  stream << "_Opacity" << result.opacity_name << "_PatternFiniteAnchors_"
         << layer_planarrgb_mul_float_variant_name(result.variant);
  result.name = stream.str();
  return result;
}

inline void PrintTo(const LayerPlanarRgbMulFloatCase& test_case, std::ostream* stream) {
  *stream << test_case.name;
}

template <typename T>
void fill_layer_planarrgb_mul_float_inputs(
    const LayerPlanarRgbMulFloatCase& test_case,
...
template <typename T>
void copy_layer_planarrgb_mul_float_planes(
    const LayerPlanarRgbMulFloatCase& test_case,
...
void apply_layer_planarrgb_mul_float_reference(
    const LayerPlanarRgbMulFloatCase& test_case,
...
void run_layer_planarrgb_mul_float_case(const LayerPlanarRgbMulFloatCase& test_case) {
  const auto destination_planes = test_case.blend_alpha ? std::size_t{4} : std::size_t{3};
  const auto overlay_planes = test_case.has_alpha ? std::size_t{4} : std::size_t{3};
  std::array<std::unique_ptr<GuardedVideoBuffer<float>>, 4> destination;
  ...
  fill_layer_planarrgb_mul_float_inputs(test_case, destination, overlay, mask);
  copy_layer_planarrgb_mul_float_planes(test_case, destination, expected);
  copy_layer_planarrgb_mul_float_planes(test_case, destination, actual);
  apply_layer_planarrgb_mul_float_reference(test_case, overlay, mask, expected);
  ...
  ASSERT_NE(test_case.variant.function, nullptr)
      << test_case.name << " AVX2 getter returned no float Planar RGB multiply function";
