...
using LayerRgb32AddFunction = void (*)(BYTE*, const BYTE*, int, int, int, int, int);

struct LayerRgb32AddCase {
  ...
  Variant<LayerRgb32AddFunction> variant;
  std::string expected_hash;
  std::string name;
};

inline std::string layer_rgb32_add_variant_name(const Variant<LayerRgb32AddFunction>& variant) {
...
inline std::string layer_rgb32_add_case_name(const LayerRgb32AddCase& test_case) {
  std::ostringstream stream;
  stream << "Rgb32_AddLuma_Width" << test_case.width_pixels << "_Height" << test_case.height
         << "_DstPitch" << test_case.destination_pitch << "_OverlayPitch" << test_case.overlay_pitch
         << "_Level" << test_case.level_name << "_PatternBoundaryAnchors_"
         << layer_rgb32_add_variant_name(test_case.variant);
  return stream.str();
}

inline LayerRgb32AddCase make_layer_rgb32_add_case(std::size_t width_pixels, std::size_t height,
                                                   std::size_t destination_pitch,
                                                   std::size_t overlay_pitch, int level,
                                                   std::string level_name,
                                                   Variant<LayerRgb32AddFunction> variant,
                                                   std::string expected_hash = {}) {
  if (width_pixels == 0 || height == 0 || destination_pitch < width_pixels * 4 ||
      overlay_pitch < width_pixels * 4) {
    throw std::invalid_argument("Layer RGB32 add rows must contain active pixels");
  }
  if (level < 0 || level > 257) {
    throw std::invalid_argument("Layer RGB32 add level must be in 0..257");
  }
  LayerRgb32AddCase result{width_pixels,
                           height,
                           destination_pitch,
                           overlay_pitch,
                           level,
                           ...
                           {}};
  result.name = layer_rgb32_add_case_name(result);
  return result;
}

inline void PrintTo(const LayerRgb32AddCase& test_case, std::ostream* stream) {
  *stream << test_case.name;
}

inline void fill_layer_rgb32_add_inputs(PlaneView<std::uint8_t> destination,
...
inline void apply_layer_rgb32_add_reference(const LayerRgb32AddCase& test_case,
...
inline void run_layer_rgb32_add_case(const LayerRgb32AddCase& test_case) {
  ...
  fill_layer_rgb32_add_inputs(destination.view(), overlay.view());
  ...
  apply_layer_rgb32_add_reference(test_case, overlay.view().as_const(), expected.view());
