...
void add_overlay_integer_variants(std::vector<OverlayIntegerCase>& cases,
                                  MaskMode mask_mode, int bits_per_pixel,
                                  std::size_t width_pixels,
                                  std::size_t height_pixels, int opacity,
                                  const std::string& opacity_label,
                                  const char* expected_hash) {
  const auto scalar = get_overlay_blend_masked_fn_c(
      mask_mode != MASK444, mask_mode);
  const auto sse41 = get_overlay_blend_masked_fn_sse41(
      mask_mode != MASK444, mask_mode);
  const auto avx2 = get_overlay_blend_masked_fn_avx2(
      mask_mode != MASK444, mask_mode);
  cases.push_back(make_overlay_integer_case(
      mask_mode, bits_per_pixel, width_pixels, height_pixels, opacity,
      opacity_label, scalar,
      Variant<OverlayMaskedFuncPtr>{"c", scalar, IsaRequirement::Scalar},
      expected_hash));
  cases.push_back(make_overlay_integer_case(
      mask_mode, bits_per_pixel, width_pixels, height_pixels, opacity,
      opacity_label, scalar,
      Variant<OverlayMaskedFuncPtr>{"sse4.1", sse41, IsaRequirement::Sse41},
      expected_hash));
  cases.push_back(make_overlay_integer_case(
      mask_mode, bits_per_pixel, width_pixels, height_pixels, opacity,
      opacity_label, scalar,
      Variant<OverlayMaskedFuncPtr>{"avx2", avx2, IsaRequirement::Avx2},
      expected_hash));
}
...
std::vector<OverlayIntegerCase> overlay_integer_cases() {
  ...
  for (const int bits_per_pixel : {8, 10, 12, 14, 16}) {
    ...
    add_overlay_integer_variants(
        cases, MASK444, bits_per_pixel, width, 5, max_value,
        ...
        : "fca3f28bfea3779b");
    add_overlay_integer_variants(
        cases, MASK444, bits_per_pixel, width, 5, max_value * 3 / 5,
  ...
  for (const auto mask_mode : {MASK420, MASK422}) {
    ...
    add_overlay_integer_variants(
        cases, mask_mode, 8, 17, 5, 255, "Max255",
        mask420 ? "f82db7d94634ce88" : "2c69e85ccb7afc35");
    add_overlay_integer_variants(
        cases, mask_mode, 8, 17, 5, 153, "Partial153",
        mask420 ? "5e45c1d988e60008" : "6b5b4d5386b1d317");
    add_overlay_integer_variants(
        cases, mask_mode, 16, 17, 5, 65535, "Max65535",
        mask420 ? "d598775ac75506ab" : "bb843dccb2f0646f");
    add_overlay_integer_variants(
        cases, mask_mode, 16, 17, 5, 39321, "Partial39321",
        mask420 ? "4e75a56021d46239" : "bcf6c37afc46ac99");
  }
  return cases;
}

void add_overlay_float_variants(std::vector<OverlayFloatCase>& cases,
                                MaskMode mask_mode, float opacity,
                                const char* opacity_label) {
  const auto scalar = get_overlay_blend_masked_float_fn_c(
      mask_mode != MASK444, mask_mode);
  const auto sse41 = get_overlay_blend_masked_float_fn_sse41(
      mask_mode != MASK444, mask_mode);
  const auto avx2 = get_overlay_blend_masked_float_fn_avx2(
      mask_mode != MASK444, mask_mode);
...
class OverlayIntegerMaskedKernels
    : public ::testing::TestWithParam<OverlayIntegerCase> {};
...
INSTANTIATE_TEST_SUITE_P(
    Kernels,
    OverlayIntegerMaskedKernels,
    ::testing::ValuesIn(overlay_integer_cases()),
    [](const ::testing::TestParamInfo<OverlayIntegerCase>& info) {
      return info.param.name;
    });

class OverlayFloatMaskedKernels
    : public ::testing::TestWithParam<OverlayFloatCase> {};
