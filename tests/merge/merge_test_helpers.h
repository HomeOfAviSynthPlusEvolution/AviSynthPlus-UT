...
inline void PrintTo(const MergeCase& test_case, std::ostream* stream) {
...
inline std::string merge_case_name(const std::string& format,
                                   int bits_per_pixel,
                                   std::size_t width_pixels,
                                   std::size_t height_pixels,
                                   std::size_t destination_pitch,
  ...
  stream << format << "Bpp" << bits_per_pixel
         << "_Width" << width_pixels
         ...
         << "_" << merge_variant_name(variant);
  return stream.str();
}

inline MergeCase make_merge_case(std::string format,
                                 int bits_per_pixel,
                                 std::size_t width_pixels,
                                 std::size_t height_pixels,
                                 std::size_t destination_pitch,
  ...
  MergeCase result{std::move(format), bits_per_pixel, width_pixels,
                   height_pixels, destination_pitch, other_pitch, weight,
                   32768 - weight, seed, scalar_function, std::move(variant),
                   std::move(expected_hash), {}};
  result.name = merge_case_name(
      result.format, result.bits_per_pixel, result.width_pixels,
...
template <typename T>
void fill_merge_random(PlaneView<T> view, std::uint32_t seed,
...
template <typename T>
void apply_merge_reference(const MergeCase& test_case,
  ...
  for (std::size_t y = 0; y < test_case.height_pixels; ++y) {
    for (std::size_t x = 0; x < test_case.width_pixels; ++x) {
      ...
      destination.row(y)[x] = static_cast<T>(
          (first * static_cast<std::uint64_t>(test_case.inverse_weight) +
...
template <typename T>
void invoke_merge(MergeFuncPtr function, PlaneView<T> destination,
                  PlaneView<const T> other, const MergeCase& test_case) {
  function(reinterpret_cast<BYTE*>(destination.data()),
           ...
           test_case.inverse_weight, test_case.bits_per_pixel);
}
...
void run_merge_case_typed(const MergeCase& test_case) {
  ...
  GuardedVideoBuffer<T> other(
      test_case.width_pixels, test_case.height_pixels, test_case.other_pitch,
      32);
  ...
  invoke_merge(test_case.scalar_function, scalar.view(), other.view().as_const(),
               test_case);
  invoke_merge(test_case.variant.function, actual.view(), other.view().as_const(),
               test_case);
