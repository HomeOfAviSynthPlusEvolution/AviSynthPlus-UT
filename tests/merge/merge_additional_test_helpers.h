...
using Yuy2MergeFunc = void (*)(BYTE*, const BYTE*, int, int, int, int, int,
                               int);
...
inline const char* yuy2_operation_name(Yuy2MergeOperation operation) {
  switch (operation) {
    case Yuy2MergeOperation::WeightedChroma: return "WeightedChroma";
    case Yuy2MergeOperation::WeightedLuma: return "WeightedLuma";
    case Yuy2MergeOperation::ReplaceLuma: return "ReplaceLuma";
  }
  return "Unknown";
}

inline Yuy2MergeCase make_yuy2_merge_case(
    Yuy2MergeOperation operation, std::size_t width_bytes,
    std::size_t height, std::size_t destination_pitch,
    std::size_t other_pitch, int weight, Yuy2MergeFunc weighted_function,
    Yuy2ReplaceFunc replace_function, std::string expected_hash) {
  ...
  stream << yuy2_operation_name(operation) << "_WidthBytes" << width_bytes
         << "_Height" << height << "_DstPitch" << destination_pitch
...
inline void fill_yuy2_inputs(PlaneView<std::uint8_t> destination,
...
inline void apply_yuy2_merge_reference(const Yuy2MergeCase& test_case,
  ...
  const bool replace_chroma =
      test_case.operation == Yuy2MergeOperation::WeightedChroma;
    ...
    for (std::size_t x = 0; x < test_case.width_bytes; ++x) {
      ...
      if (test_case.operation == Yuy2MergeOperation::ReplaceLuma) {
        ...
        destination.row(y)[x] = static_cast<std::uint8_t>(
            (second * test_case.weight + first * test_case.inverse_weight +
             16384) >>
            15);
      }
    }
...
inline void run_yuy2_merge_case(const Yuy2MergeCase& test_case) {
  GuardedVideoBuffer<std::uint8_t> destination(
      test_case.width_bytes, test_case.height, test_case.destination_pitch, 64);
  ...
  if (test_case.operation == Yuy2MergeOperation::ReplaceLuma) {
    test_case.replace_function(
        ...
        static_cast<int>(test_case.other_pitch),
        static_cast<int>(test_case.width_bytes),
    ...
    test_case.weighted_function(
        ...
        static_cast<int>(test_case.other_pitch),
        static_cast<int>(test_case.width_bytes),
        static_cast<int>(test_case.height), test_case.weight,
        test_case.inverse_weight);
  }

  EXPECT_TRUE(compare_exact(expected.view().as_const(),
                            destination.view().as_const()))
      << test_case.name;
  EXPECT_EQ(format_hash(hash_active(destination.view().as_const())),
...
inline const char* average_sample_name(AverageSample sample) {
  switch (sample) {
    case AverageSample::UInt8: return "UInt8";
    case AverageSample::UInt16: return "UInt16";
    case AverageSample::Float: return "Float";
  }
  return "Unknown";
}

inline AveragePlaneCase make_average_plane_case(
    AverageSample sample, std::size_t width, std::size_t height,
    std::size_t destination_pitch, std::size_t other_pitch,
    std::size_t alignment_offset, Variant<AveragePlaneFunc> variant,
    std::string expected_hash = {}) {
  AveragePlaneCase result{sample,
                          ...
                          {}};
  std::ostringstream stream;
  stream << average_sample_name(sample) << "_Width" << width << "_Height"
         << height << "_DstPitch" << destination_pitch << "_OtherPitch"
         << other_pitch << "_AlignOffset" << alignment_offset
         << (sample == AverageSample::Float ? "_PatternFiniteAnchors"
                                            : "_PatternBoundaryValues")
         << "_Variant";
  bool capitalize = true;
  for (const char character : result.variant.name) {
    stream << static_cast<char>(capitalize && character >= 'a' && character <= 'z'
                                  ? character - ('a' - 'A')
                                  : character);
...
void fill_average_inputs(PlaneView<T> destination, PlaneView<T> other) {
  ...
  constexpr std::uint32_t anchors[] = {0, 1, 2, 127, 128, 254, 255, 65534,
                                       65535};
  for (std::size_t y = 0; y < destination.height(); ++y) {
    for (std::size_t x = 0; x < destination.width(); ++x) {
      destination.row(y)[x] =
          static_cast<T>(anchors[(x + 2 * y) % std::size(anchors)]);
      other.row(y)[x] =
          static_cast<T>(anchors[(4 * x + y + 5) % std::size(anchors)]);
    }
  }
}

inline void fill_average_float_inputs(PlaneView<float> destination,
                                      PlaneView<float> other) {
  constexpr float anchors[] = {-1000.5F, -17.25F, -0.125F, 0.0F,
                               0.125F,   31.75F,  255.5F, 4096.0F};
  for (std::size_t y = 0; y < destination.height(); ++y) {
    for (std::size_t x = 0; x < destination.width(); ++x) {
      destination.row(y)[x] = anchors[(x + 3 * y) % std::size(anchors)];
...
void run_average_integer_case(const AveragePlaneCase& test_case) {
  GuardedVideoBuffer<T> destination(test_case.width, test_case.height,
                                    test_case.destination_pitch, 64,
                                    test_case.alignment_offset);
  GuardedVideoBuffer<T> other(test_case.width, test_case.height,
                              test_case.other_pitch, 64,
                              test_case.alignment_offset);
  GuardedVideoBuffer<T> expected(test_case.width, test_case.height,
                                 test_case.destination_pitch, 64,
                                 test_case.alignment_offset);
  ...
  for (std::size_t y = 0; y < test_case.height; ++y) {
    for (std::size_t x = 0; x < test_case.width; ++x) {
      expected.view().row(y)[x] = static_cast<T>(
          (static_cast<std::uint32_t>(expected.view().row(y)[x]) +
  ...
  test_case.variant.function(
      ...
      static_cast<int>(test_case.destination_pitch),
      static_cast<int>(test_case.other_pitch),
      static_cast<int>(test_case.width * sizeof(T)),
      static_cast<int>(test_case.height));
  EXPECT_TRUE(compare_exact(expected.view().as_const(),
                            destination.view().as_const()))
      << test_case.name;
...
inline void run_average_float_case(const AveragePlaneCase& test_case) {
  GuardedVideoBuffer<float> destination(test_case.width, test_case.height,
                                        test_case.destination_pitch, 64,
                                        test_case.alignment_offset);
  GuardedVideoBuffer<float> other(test_case.width, test_case.height,
                                  test_case.other_pitch, 64,
                                  test_case.alignment_offset);
  ...
  for (std::size_t y = 0; y < test_case.height; ++y) {
    for (std::size_t x = 0; x < test_case.width; ++x) {
      expected.view().row(y)[x] = static_cast<float>(
          (static_cast<double>(expected.view().row(y)[x]) +
  ...
  EXPECT_TRUE(compare_float_ulp(expected.view().as_const(),
                                destination.view().as_const()))
      << test_case.name;
  EXPECT_TRUE(other.active_matches(other_snapshot)) << test_case.name;
  EXPECT_TRUE(destination.memory_intact()) << test_case.name;
