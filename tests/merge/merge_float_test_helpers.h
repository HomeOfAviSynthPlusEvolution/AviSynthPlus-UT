...
inline std::string merge_float_variant_name(
...
inline std::string float_pattern_name(FloatInputPattern pattern) {
  switch (pattern) {
    case FloatInputPattern::RandomBounded: return "Random";
...
inline std::string merge_float_case_name(
    const std::string& pattern, std::size_t width_pixels,
    std::size_t height_pixels, std::size_t destination_pitch,
    std::size_t other_pitch, const std::string& weight_label,
    std::uint32_t seed, const Variant<MergeFloatFuncPtr>& variant) {
  ...
  stream << pattern
         ...
         << "_Weight" << weight_label
         << "_Seed" << std::uppercase << std::hex << seed
         << "_" << merge_float_variant_name(variant);
  return stream.str();
}

inline MergeFloatCase make_merge_float_case(
    FloatInputPattern input_pattern, std::size_t width_pixels,
    std::size_t height_pixels, std::size_t destination_pitch,
    std::size_t other_pitch, float weight, std::string weight_label,
  ...
  MergeFloatCase result{pattern, input_pattern, width_pixels, height_pixels,
                        destination_pitch, other_pitch, weight,
                        std::move(weight_label), seed, scalar_function,
                        std::move(variant), {}};
  result.name = merge_float_case_name(
      result.pattern, result.width_pixels, result.height_pixels,
      result.destination_pitch, result.other_pitch, result.weight_label,
...
inline void fill_merge_float_inputs(const MergeFloatCase& test_case,
                                    PlaneView<float> destination,
                                    PlaneView<float> other) {
  if (destination.width() != test_case.width_pixels ||
      destination.height() != test_case.height_pixels ||
      other.width() != test_case.width_pixels ||
  ...
  constexpr std::array<float, 13> anchors{
      -1024.0F, -768.0F, -512.0F, -64.0F, -1.0F, -0.001F, 0.0F,
...
inline void apply_merge_float_reference(const MergeFloatCase& test_case,
  ...
  for (std::size_t y = 0; y < test_case.height_pixels; ++y) {
    for (std::size_t x = 0; x < test_case.width_pixels; ++x) {
      ...
      destination.row(y)[x] = static_cast<float>(
          first * static_cast<double>(inverse_weight) +
          second * static_cast<double>(test_case.weight));
    }
  }
}

inline void invoke_merge_float(MergeFloatFuncPtr function,
                               PlaneView<float> destination,
                               PlaneView<const float> other,
                               const MergeFloatCase& test_case) {
  function(reinterpret_cast<BYTE*>(destination.data()),
           ...
           static_cast<int>(other.pitch_bytes()),
           static_cast<int>(test_case.width_pixels),
           static_cast<int>(test_case.height_pixels), test_case.weight);
}
...
inline ::testing::AssertionResult compare_float_ulp(
...
inline void run_merge_float_case(const MergeFloatCase& test_case) {
  ...
  GuardedVideoBuffer<float> other(
      test_case.width_pixels, test_case.height_pixels,
  ...
  invoke_merge_float(test_case.scalar_function, scalar.view(),
                     other.view().as_const(), test_case);
  invoke_merge_float(test_case.variant.function, actual.view(),
                     other.view().as_const(), test_case);
